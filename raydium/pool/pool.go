...
import (
	...
	"github.com/ALen-404/go-solana/raydium/constants"
...
func (p *Pool) getProgramAccounts(mint1 string, mint2 string) (rpc.GetProgramAccountsResult, error) {
    ...
    // 尝试执行 RPC 请求
    result, err := p.connection.GetProgramAccountsWithOpts(context.Background(), constants.RAYDIUM_V4_PROGRAM_ID, &rpc.GetProgramAccountsOpts{
    ...
    // 错误处理和打印
    if err != nil {
        fmt.Println("Error fetching program accounts:", err)
        return rpc.GetProgramAccountsResult{}, err
    }
