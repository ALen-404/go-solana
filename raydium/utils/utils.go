...
func FromWei(amount string, decimals int64) (string, error) {
    parsedDes := new(big.Int).Exp(big.NewInt(10), big.NewInt(decimals), nil)

    // Convert the amount to big.Int
    amountInt, success := new(big.Int).SetString(amount, 10)
    if !success {
        return "", fmt.Errorf("invalid amount: %s", amount)
    }

    // Divide by 10^decimals to convert to the standard unit
    amountInt.Div(amountInt, parsedDes)

    return amountInt.String(), nil
}

func ToWei(amount string, decimals int64) (string, error) {
    parsedDes := new(big.Int).Exp(big.NewInt(10), big.NewInt(decimals), nil)
    ...
    // Multiply by 10^decimals to convert to Wei (smallest unit)
    amountInt.Mul(amountInt, parsedDes)
